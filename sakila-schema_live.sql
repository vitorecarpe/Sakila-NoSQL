...
-- sakilla scheme converted to Oracle Live SQL
-- tenho umas duvidas na tradução para o Live SQL. Enviei mail ao stor. Aguardando resposta...

CREATE SCHEMA AUTHORIZATION sakila
...
CREATE TABLE staff (
  ...
  CONSTRAINT "store_FK" PRIMARY KEY store("store_id"),
  CONSTRAINT "address_FK" PRIMARY KEY address("address_id")
);
...
CREATE TABLE IF NOT EXISTS `sakila`.`film_actor` (
  ...
  INDEX `fk_film_actor_actor` (`actor_id` ASC),
...
CREATE TABLE IF NOT EXISTS `sakila`.`film_category` (
  ...
  INDEX `fk_film_category_film` (`film_id` ASC),
...
CREATE TABLE IF NOT EXISTS `sakila`.`inventory` (
  ...
  INDEX `fk_inventory_store` (`store_id` ASC),
