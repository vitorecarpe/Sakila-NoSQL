...
mongoimport.exe --db nosql --type csv --file "./csv/customer.csv" --fields "customer_id","store_id","name","email","address_id","active","create_date","last_update"
mongoimport.exe --db nosql --type csv --file "./csv/address_city_country.csv" --fields "address_id","address","district","postal_code","phone","location","city","country","last_update"

#juntar o address embebido ao customer
mongo.exe nosql --eval "db.customer.aggregate([
    {\$lookup: {
